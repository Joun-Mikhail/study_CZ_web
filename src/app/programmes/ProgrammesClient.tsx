...
function ProgrammeCard({ prog, locale }: { prog: Programme; locale: "en" | "ar" }) {
  ...
  return (
    <GlassCard className="group relative" hoverEffect="border">
  ...
  return (
    <div className="min-h-screen">
      ...
      <main id="main-content" className="max-w-5xl mx-auto px-4 pt-24 pb-16">
        ...
        <div className="flex gap-2 mb-4">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full ps-10 pe-4 py-2.5 rounded-xl bg-white/5 border border-border-subtle text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:border-amber/40 transition-colors"
            />
          </div>
          <button
        ...
        {/* Filter panel */}
        {showFilters && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6 p-4 rounded-xl bg-white/[0.02] border border-border-subtle">
            <div>
            ...
            {activeFilters > 0 && (
              ...
              >
                <X className="w-3 h-3" /> {locale === "ar" ? "مسح الفلاتر" : "Clear all filters"}
              </button>
            )}
          </div>
        )}
        ...
        {filtered.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filtered.map((prog) => (
              <ProgrammeCard key={prog.id} prog={prog} locale={locale} />
            ))}
          </div>
        ) : (
          <div className="text-center py-16">
            ...
            <h3 className="text-lg font-semibold text-text-primary mb-2">
              {locale === "ar" ? "لا توجد نتائج" : "No programmes found"}
            </h3>
